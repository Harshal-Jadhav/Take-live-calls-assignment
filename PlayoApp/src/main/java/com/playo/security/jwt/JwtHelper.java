package com.playo.security.jwt;
...
public class JwtHelper {
	...
	public ResponseCookie generateCookie(User user) {
		...
		ResponseCookie cookie = ResponseCookie.from(cookieName, authToken).path("/playo").maxAge(24 * 60 * 60)
				.httpOnly(true).build();
		return cookie;
	}

	public ResponseCookie deleteCookie() {
		ResponseCookie cookie = ResponseCookie.from(cookieName, null).path("/playo").build();
		return cookie;
	}
